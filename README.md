...
| Agent Connector (GPT / Gemini / DeepSeek / Groq) | ✅ |
| Business Workflow (`genesis plan`) | ✅ |
| Q&A over knowledge base (`genesis ask`) | ✅ |
...
## Asking Questions (RAG-backed Q&A)

```bash
# Ask a question — Genesis Engine retrieves context from the RAG and answers via LLM
genesis ask "what is the target audience?"
genesis ask "what are the main revenue streams?"
genesis ask "what features are in the MVP?"

# Restrict context to a specific namespace
genesis ask "what is the go-to-market strategy?" --namespace business

# Use more RAG context documents for complex questions
genesis ask "how does the competitive landscape look?" -k 10
```

The answer is generated by the configured LLM (from `genesis.yaml`), grounded in the actual knowledge base content.
